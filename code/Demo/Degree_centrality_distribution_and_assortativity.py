...
def fitting(data, full_data=True, x_min=None, x_max=None, survival=True):
    """
    Use package 'powerlaw' to fit data.
    A fit of a data set to various probability distributions,
    namely power laws. For fits to power laws, the methods of Clauset et al. 2007 are used.
    These methods identify the portion of the tail of the distribution that follows a power law,
    beyond a value xmin. If no xmin is provided, the optimal one is calculated and assigned at initialization.

    :param data: pandas.Series or list
    :param full_data: Bool, default True, if False: fit the power-law
                        distribution p(x) ~ x^-alpha for x >= x_min; True: use full data set to fit
    :param x_min: int or float, the data value beyond which distributions should be fitted.
                  If None an optimal one will be calculated.
    :param x_max: int or float, The maximum value of the fitted distributions.
    :return:
    """
    if full_data:
        x_min = data.min()
