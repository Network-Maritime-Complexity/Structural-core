...
def plfit(x, *varargin):
    ...
    if f_dattype == 'REAL':
        ...
        xmins = xmins[0:-1]
        if xminx != []:
            xmins = [min(list(filter(lambda X: X >= xminx, xmins)))]
        ...
        for xm in range(0, len(xmins)):
            ...
            # cx   = list(map(lambda X:float(X)/n,range(0,n)))
        ...
        for xm in range(0, len(xmins)):
            ...
            for k in range(0, len(vec)):
                L.append(-vec[k] * float(slogz) - float(n) * log(
                    float(zvec[k]) - sum(list(map(lambda X: pow(float(X), -vec[k]), xminvec)))))

            I = L.index(max(L))
            # compute KS statistic
            fit = reduce(lambda X, Y: X + [Y + X[-1]], \
                         (list(map(lambda X: pow(X, -vec[I]) / (
                                     float(zvec[I]) - sum(list(map(lambda X: pow(X, -vec[I]), list(map(float, range(1, xmin))))))),
        ...
        L = -alpha * sum(list(map(log, z))) - n * log(zvec[vec.index(max(list(filter(lambda X: X <= alpha, vec))))] - \
                                                sum(list(map(lambda X: pow(X, -alpha), range(1, xmin)))))
    else:
