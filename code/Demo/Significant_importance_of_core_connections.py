...
class LFCConnection:
    ...
    def core_connections_importance(self):
        ...
        print()
        print('"First, core connections themselves tend to be longer than feeder and local connections (Fig. 9b). '
            ...
            dis_mean['feeder'], dis_std['feeder'], dis_mean['core'] / dis_mean['feeder'], dis_mean['local'],
            dis_std['local'], dis_mean['core'] / dis_mean['local']))
        print()

        if SAVE_RESULT:
            save_path = os.path.join('output', 'Significant_importance_of_core_connections')
            if os.path.exists(save_path):
                pass
            else:
                os.makedirs(save_path)
            filename = 'Fig. 9 Statistics of the core feeder and local connections (b).xlsx'
    ...
    def pr_lfc(edges):
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            filename = 'Fig. 9 Statistics of the core feeder and local connections (c).xlsx'
            df_res_c.to_excel(save_path + '/' + filename, index=False, na_rep=0.0)
            print('The result file "{}" saved at: "{}"'.format(filename, save_path))
            print()
...
def startup():
    ...
    print('Subsection titled "Significant importance of core connections in '
          'supporting long-distance maritime transportation"')
