...
def plot_hist(y_c, tv_mean, tv_se, y_cp, btv_mean, btv_se):
    ...
    x = [2, 4, 6, 8]
    y_c.append(0.0)
    ax1.bar(x, y_c, color=['#333333', '#f57f17', '#339933', 'white'], alpha=0.8, width=0.6)
    ...
    ax1.set_ylabel('Pearson correlation coefficient', fontsize=22, labelpad=10)
    ax1.set_ylim([0, 1])
    ax1.set_title('a', fontsize=26, weight='bold', pad=10, loc='left')

    y_cp.append(0.0)
    ax2 = fig.add_subplot(122)
    ax2.bar(x, y_cp, color=['#333333', '#f57f17', '#339933', 'white'], alpha=0.8, width=0.6)

    ax2.bar(8, btv_mean, color='gray', alpha=0.4, width=0.6)
    ax2.errorbar(8, btv_mean, yerr=btv_se, marker='', fmt='k', lw=2, capsize=12)
    ax2.set_ylim([0, 1])
    ax2.set_title('b', fontsize=26, weight='bold', pad=10, loc='left')
    label_font = {'family': 'Arial', 'weight': 'medium', 'size': 24}
    ...
    save_path = os.path.join('output', 'Structural_core_and_international_trade')
    if SAVE_RESULT:
        if os.path.exists(save_path):
            pass
        else:
            os.makedirs(save_path)
        filename = "Fig. 9 Correlation between the GLSN topological indicators and international trade " \
                   "indicators of countries.png"
...
def startup():
    ...
    print('Subsection titled "Structural core and international trade"')
