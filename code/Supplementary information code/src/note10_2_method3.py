...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 23 Frequency...percent of edges-method #3-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 23 Frequency...percent of edges-method #3-(b) GLSN of 2017.png'
...
def permutation_heat_map(year, iterations):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 21 Edge distributions...(e).png'
        else:
            filename = 'Supplementary Fig. 22 Edge distributions...(e).png'
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../', 'data', 'note10_2', year, 'method3', str(perc))
            all_freq = []
            country_pr = []
            country_pair_pr = []
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary Fig. 21 Edge distributions....(b)-method #3.xlsx'
                s_name = 'GLSN of 2015_method #3'
            else:
                filename = 'Supplementary Fig. 22 Edge distributions....(b)-method #3.xlsx'
