...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 19 Frequency...removed routes-random removal-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 19 Frequency...removed routes-random removal-(b) GLSN of 2017.png'
...
def plot_hist(year, tv_mean, tv_se, btv_mean, btv_se, iterations):
    ...
    if SAVE_RESULT:
        ...
        if os.path.exists(save_path):
            ...
            os.makedirs(save_path)
        if year == '2015':
            filename = 'Supplementary Fig. 20 Pearson correlation coefficients...' \
                       'removed routes-(b).png'
        else:
            filename = 'Supplementary Fig. 20 Pearson correlation coefficients...' \
                       'removed routes-(d).png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../data', 'note10_1', year, 'RandomRemoval', str(perc))
