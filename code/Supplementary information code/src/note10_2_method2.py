...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 23 Frequency...percent of edges-method #2-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 23 Frequency...percent of edges-method #2-(b) GLSN of 2017.png'
...
def plot_hist(year, tv_mean, tv_se, btv_mean, btv_se, iterations):
    ...
    axes[0].set_title('(b) GLSN of {}, method #2'.format(year), loc='left', fontsize=24, pad=10)
...
def permutation_heat_map(year, iterations):
    ...
    for percentage in percentage_list:
        data = pd.read_csv('output/dis_process/Permutation-KK-Dis_' + year + '_method2_' + str(percentage) + '.csv')
    ...
    plt.suptitle('Note: The number of iterations of the experiment: in your test, {}; in the '
                 'manuscript, 1000.'.format(iterations) + '\n' + '\n' + '(d) modified GLSNs of {} by method #2'.format(year), color='k', fontsize=22, x=0.5, y=1.25)
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 21 Edge distributions...(d).png'
        else:
            filename = 'Supplementary Fig. 22 Edge distributions...(d).png'
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../', 'data', 'note10_2', year, 'method2', str(perc))
            all_freq = []
            country_pr = []
            country_pair_pr = []
            ...
            if os.path.exists(spath):
                ...
                os.makedirs(spath)
            df_rand_kk.to_csv(spath + '/Permutation-KK-Dis_' + year + '_method2_' + str(perc) + '.csv',
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary Fig. 21 Edge distributions....(b)-method #2.xlsx'
                s_name = 'GLSN of 2015_method #2'
            else:
                filename = 'Supplementary Fig. 22 Edge distributions....(b)-method #2.xlsx'
                s_name = 'GLSN of 2017_method #2'
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary Table 8 Pearson correlation coefficients...2015...-method #2.xlsx'
                s_name = 'GLSN of 2015_method #2'
            else:
                filename = 'Supplementary Table 9 Pearson correlation coefficients...2017...-method #2.xlsx'
                s_name = 'GLSN of 2017_method #2'
