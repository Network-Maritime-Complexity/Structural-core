...
data_path = os.path.join('../data', '1000 Community divisions')
...
def sc_robustness():
    ...
    for rt in range(1, iters+1):
        ...
        node_data['Community'] = node_data['id'].apply(dict_module.get)
        df_nodes = cal_bzp(node_data)
...
def plot_js_error(data):
    ...
    if SAVE_RESULT:
        ...
        filename = 'Supplementary Fig. 7 Overlap between the rich club and the structural core of the GLSN (a).png'
...
def startup():
    if os.path.exists(data_path):
        ...
        print('This script reproduce the result figure titled "Supplementary Fig. 7 Overlap between the rich club '
              'and the structural core of the GLSN"')
        print('*********************************')
        print()
        print()
