...
def calculate_intra_inter_connections_percentage():
    ...
    inter_mean_sr = dict_dis[0]['mean'][0]
    inter_std_sr = dict_dis[0]['std'][0]
    intra_mean_sr = dict_dis[0]['mean'][1]
    intra_std_sr = dict_dis[0]['std'][1]

    inter_mean_gc = dict_dis[1]['mean'][0]
    inter_std_gc = dict_dis[1]['std'][0]
    intra_mean_gc = dict_dis[1]['mean'][1]
    intra_std_gc = dict_dis[1]['std'][1]
    print('The in-text result:')
    print()
    print('"The average length of inter-module links is {:.0f} km (SD = {:.0f} km) and that of intra-module '
          'links is {:.0f} km (SD = {:.0f} km), measured by real nautical distance; {:.0f} km (SD = {:.0f} km ) '
          'and {:.0f} km (SD = {:.0f} km), measured by '
          'great-circle distance."'.format(inter_mean_sr, inter_std_sr, intra_mean_sr, intra_std_sr,
                                           inter_mean_gc, inter_std_gc, intra_mean_gc, intra_std_gc))
    print()
...
def plot_result(data):
    ...
    if SAVE_RESULT:
        save_path = os.path.join('output', 'Supplementary note 3')
        if os.path.exists(save_path):
            pass
        else:
            os.makedirs(save_path)
        filename = 'Supplementary Fig. 2 Proportional distribution of intra- and inter- module ' \
                   'links in different range of geographical length.png'
...
def startup():
    ...
    print('Section titled "Supplementary note 3: Geographical length of inter-port links in the GLSN"')
    print('*********************************')
    print()
    print('---------------------------------------------------------------------------------------------------')
    print('Output:')
