...
def empirical_heat_map(data, df_nodes, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 18 Edge distributions...(a).png'
        else:
            filename = 'Supplementary Fig. 19 Edge distributions...(a).png'
...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if os.path.exists(save_path):
            ...
            os.makedirs(save_path)
        if year == '2015':
            filename = 'Supplementary Fig. 20 Frequency...percent of edges-method #1-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 20 Frequency...percent of edges-method #1-(b) GLSN of 2017.png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
        print('The result file "{}" saved at: "{}"'.format(filename, save_path))
        print()
...
def permutation_heat_map(year, iterations):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 18 Edge distributions...(c).png'
        else:
            filename = 'Supplementary Fig. 19 Edge distributions...(c).png'
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../', 'data', 'downloaded data files', 'note12_2', year, 'method1', str(perc))
            all_freq = []
            country_pr = []
            country_pair_pr = []
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary Fig. 18 Edge distributions....(b)-method #1.xlsx'
                s_name = 'GLSN of 2015_method #1'
            else:
                filename = 'Supplementary Fig. 19 Edge distributions....(b)-method #1.xlsx'
