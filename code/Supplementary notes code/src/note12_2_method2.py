...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 20 Frequency...percent of edges-method #2-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 20 Frequency...percent of edges-method #2-(b) GLSN of 2017.png'
...
def permutation_heat_map(year, iterations):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 18 Edge distributions...(d).png'
        else:
            filename = 'Supplementary Fig. 19 Edge distributions...(d).png'
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../', 'data', 'downloaded data files', 'note12_2', year, 'method2', str(perc))
            all_freq = []
            country_pr = []
            country_pair_pr = []
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary Fig. 18 Edge distributions....(b)-method #2.xlsx'
                s_name = 'GLSN of 2015_method #2'
            else:
                filename = 'Supplementary Fig. 19 Edge distributions....(b)-method #2.xlsx'
