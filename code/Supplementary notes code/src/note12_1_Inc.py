...
def plot_sc_freq(data, iterations, year):
    ...
    if SAVE_RESULT:
        ...
        if year == '2015':
            filename = 'Supplementary Fig. 16 Frequency...removed routes-Increasing removal-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 16 Frequency...removed routes-Increasing removal-(b) GLSN of 2017.png'
...
def plot_hist(year, tv_mean, tv_se, btv_mean, btv_se, iterations):
    ...
    if SAVE_RESULT:
        ...
        if os.path.exists(save_path):
            ...
            os.makedirs(save_path)
        if year == '2015':
            filename = 'Supplementary Fig. 17 Pearson correlation coefficients...socio-economic...removed routes-(a).png'
        else:
            filename = 'Supplementary Fig. 17 Pearson correlation coefficients...socio-economic...removed routes-(c).png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
        print('The result file "{}" saved at: "{}"'.format(filename, save_path))
    else:
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../data', 'downloaded data files', 'note12_1', year, 'IncreasingRemoval')
