...
def plot_sc_freq(data, iterations, year):
    ...
    for i in range(n_cols):
        ...
        ax.bar(x, data_copy, width, color='gray', alpha=0.75, label='Random removal')
    ...
    if SAVE_RESULT:
        save_path = os.path.join('output', 'Supplementary note 12_1')
        if os.path.exists(save_path):
            pass
        else:
            os.makedirs(save_path)
        if year == '2015':
            filename = 'Supplementary Fig. 16 Frequency...removed routes-random removal-(a) GLSN of 2015.png'
        else:
            filename = 'Supplementary Fig. 16 Frequency...removed routes-random removal-(b) GLSN of 2017.png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
        print('"{}" saved at: "{}"'.format(filename, save_path))
    else:
        plt.show()
    plt.close('all')
...
def plot_hist(year, tv_mean, tv_se, btv_mean, btv_se, iterations):
    ...
    if year == '2015':
        axes[0].set_title('(b) GLSN of 2015, random removal', fontsize=24, pad=10)
    else:
        axes[0].set_title('(d) GLSN of 2017, random removal', fontsize=24, pad=10)
    ...
    if SAVE_RESULT:
        ...
        if os.path.exists(save_path):
            ...
            os.makedirs(save_path)
        if year == '2015':
            filename = 'Supplementary Fig. 17 Pearson correlation coefficients...' \
                       'socio-economic...removed routes-(b).png'
        else:
            filename = 'Supplementary Fig. 17 Pearson correlation coefficients...' \
                       'socio-economic...removed routes-(d).png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
...
def sc_robustness(iterations):
    ...
    for year in years:
        ...
        for perc in percs:
            data_path = os.path.join('../data', 'downloaded data files', 'note12_1', year, 'RandomRemoval', str(perc))
        ...
        if SAVE_RESULT:
            ...
            if os.path.exists(save_path):
                ...
                os.makedirs(save_path)
            if year == '2015':
                filename = 'Supplementary table 6 Pearson correlation coefficients...2015-Random removal.xlsx'
                s_name = 'GLSN of 2015_Random removal'
            else:
                filename = 'Supplementary table 7 Pearson correlation coefficients...2017-Random removal.xlsx'
                s_name = 'GLSN of 2017_Random removal'
        ...
        if df_country_pr_all.empty or df_country_pair_pr_all.empty:
            ...
            country_pair_pr_mean = \
            df_country_pair_pr_all.groupby(['percent of removed routes'])[
                '# all connections', '# SC connections', '# NSC connections'].apply(np.mean).round(2)
