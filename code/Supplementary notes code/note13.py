...
lat_path = os.path.join('../data', 'downloaded data files', 'note13', 'Lattice model')
rand_path = os.path.join('../data', 'downloaded data files', '1000 equivalent random networks')
...
def plot_res(l_rand, c_latt):
    ...
    if SAVE_RESULT:
        save_path = os.path.join('output', 'Supplementary note 13')
        if os.path.exists(save_path):
            pass
        else:
            os.makedirs(save_path)
        filename = 'Supplementary Fig. 23 Small-world networks generated by adoption of the Watts-Strogatz...(b).png'
...
def plot_net():
    ...
    if SAVE_RESULT:
        ...
        if os.path.exists(save_path):
            ...
            os.makedirs(save_path)
        filename = 'Supplementary Fig. 23 Small-world networks generated by adoption of the Watts-Strogatz...(a).png'
        plt.savefig(save_path + '/' + filename, bbox_inches='tight')
        print()
        print('The result file "{}" saved at: "{}"'.format(filename, save_path))
        print()
...
def startup():
    if os.path.exists(lat_path) and os.path.exists(rand_path) and os.path.exists(sw_path):
        ...
        print('Section titled "Supplementary note 13: Existence of a structural core of the GLSN is not '
        ...
        from src import note13_1
        from src import note13_2
        from src import note13_3
        from src import note13_4
        p1 = mp.Process(target=note13_1.startup, args=(iters, ))
        p2 = mp.Process(target=note13_2.startup, args=(iters, ))
        p3 = mp.Process(target=note13_3.startup, args=(iters, ))
        p4 = mp.Process(target=note13_4.startup, args=(iters, ))
        p1.start()
        p2.start()
        p3.start()
        p4.start()
